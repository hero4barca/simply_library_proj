...
from library.models import Book, Author  # Import your models
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        with open(csv_file, newline='', encoding='utf-8') as file:
            ...
            with transaction.atomic():
                for row in reader:
                    ...
                    language = row['language'].strip() if row['language'] else ''
                    work_id = row['work_id'].strip() if row['work_id'] else ''
                    edition_information = row['edition_information'].strip() if row['edition_information'] else ''
                    publisher = row['publisher'].strip() if row['publisher'] else ''
                    num_pages = row['num_pages'].strip() if row['num_pages'] else None
                    series_id = row['series_id'].strip() if row['series_id'] else ''
                    series_name = row['series_name'].strip() if row['series_name'] else ''
                    series_position = row['series_position'].strip() if row['series_position'] else ''
        ...
        self.stdout.write(self.style.SUCCESS(f"load report== created:{create}, updated:{update}, errors:{errors}"))
